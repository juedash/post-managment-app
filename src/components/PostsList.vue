  ...
  <section class="container-xl lg:container px-2 md:mx-auto my-6">
    ...
    <div v-else>
      <div v-if="postState.posts.length">
        ...
        <div
          class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mt-12 mb-6"
        >
          <PostListItem
...
const filteredPosts = computed(() => {
  ...
  return postState.posts.filter((post: PostItem) => {
    const matchesUser = !selectedUser.value || post?.userId === selectedUser.value
    const matchesSearch = !searchQuery.value || post?.title?.toLowerCase().includes(searchLower)
